...
function highestCount(arr) {
  return arr.sort((a, b) => b - a).filter((...[element, , arr]) => element === arr[0]).length;
}
...
function triangleCheck(lineA, lineB, lineC) {
  return [...arguments].filter((...[line,,arr]) => line < arr.reduce((acc, val) => acc + val) - line).length === 3;
}

// Desafio 13
function hydrate(str) {
  const waterCounter = str.match(/\d+/g).reduce((a, b) => parseInt(a) + parseInt(b));
